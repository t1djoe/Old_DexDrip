...
/**
 * Created by joe on 1/25/15.
 */
public class IOB_COB {

    public static double getCOB(Context context) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        Cursor cursor = Cache.openDatabase().rawQuery("Select * from Treatments", null);

        double carbRatio = Double.parseDouble(prefs.getString("carbRatio", "10.0"));
        double carbs_hr = Double.parseDouble(prefs.getString("carbs_hr", "30.0"));
        double basal = Double.parseDouble(prefs.getString("basal", "1.0"));
        double insulinDIA = Double.parseDouble(prefs.getString("insulinDIA", "3"));
        double sens = Double.parseDouble(prefs.getString("sensitivity", "20.0"));
        ...
        public void cobTotal(Treatments treatments, Profile profile) {
            double cob;
            double carbImpact;
            double isDecaying;
            double nextCarbTreatment;
            ...
            double carbs_hr = profile.carbs_hr;
